...
# ---- Initial parameters at snap 0 to initialize the functions ----
ds_init = ts[0]   # fist dataset
...
Lbox_3d = ts[0].domain_right_edge.to('Mpc').v - ts[0].domain_left_edge.to('Mpc').v
Lbox_x = Lbox_3d[0]
Lbox_y = Lbox_3d[1]
Lbox_z = Lbox_3d[2]
...
x_ref_cont = ts[0].domain_left_edge.to('Mpc').v[0] + (np.argmax(gdx_init) + 0.5) * dx_init

gdy_init = np.sum(cg_init['boxlib', 'gasDensity'], axis=(0,2))
dy_init = Lbox_y / ds_init.domain_dimensions[1]
y_ref_cont = ts[0].domain_left_edge.to('Mpc').v[1] + (np.argmax(gdy_init) + 0.5) * dy_init

gdz_init = np.sum(cg_init['boxlib', 'gasDensity'], axis=(0,1))
dz_init = Lbox_z / ds_init.domain_dimensions[2]
z_ref_cont = ts[0].domain_left_edge.to('Mpc').v[2] + (np.argmax(gdz_init) + 0.5) * dz_init

print("\n" + "-"*58)
print(f"Start analysing {len(ts)} quokka dataset of the simulation...\n")


for i, ds in enumerate(ts):        
    plt_path = ds.parameter_filename       # absolute path of the output
    plt_name = os.path.basename(plt_path)  # extract the tail of the path (plt*)
    ...
    if os.path.exists(metadata_file):     # if the file exists, open it in reading mode
        ...
        a_now   = float(cosmo.get('a'))           # float for scientific notation compatibility
        z_now   = cosmo.get('z')
    ...
    # Correct way to perform global integrals in AMR with yt (avoids double counting of coarse/fine cells)
    # total_momentum_x = ad.quantities.total_quantity(("gas", "momentum_x"))
    # total_mass       = ad.quantities.total_quantity(("gas", "cell_mass"))
   
    # x_coordinates = ad['index', 'x'].to('Mpc').v   # old flat extraction for cells [(x0, y0, z0), (x0, y0, z1), ...]
    ...
    if total_gas_density > 0:
        ...
        # gas_center_x = np.average(x_coordinates, weights=gdx)
        # gas_center_y = np.average(y_coordinates, weights=gdy)
        # gas_center_z = np.average(z_coordinates, weights=gdz)
...
print(f"DM particle drifted for {tot_dm_drift:.3f} cMpc during the simuation span (z={z_values[0]:.1f} to z={z_values[-1]:.1f})")
...
if len(density_frames) > 1:
    ...
    # Initialize the object reading the first file 
    first_png_path = density_frames[0]
    img_obj = ax_anim.imshow(mgimg.imread(first_png_path))
    fig_anim.tight_layout()

    def update_density_slice(frame):
