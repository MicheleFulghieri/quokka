...
# 2b. Relative error
error_rel_pct = np.abs(a_values - a_theoretical) / a_theoretical * 100
print(f"Max consistency error for a(t): {np.max(error_rel_pct):.2e}%")
...
ax_ins.set_title(r"Scale factor RE: $\frac{a_{\mathrm{sim}} - a_{\mathrm{pre}}}{a_{\mathrm{sim}}}$", fontsize=9)
...
# 2g. Text insertion in the figure
...
ax.tick_params(direction='in', which='both')     # ticks point inward
ax.grid(True, which='both', linestyle=':', alpha=0.5)
ax.legend(loc='best', bbox_to_anchor=(0.5, 0.70))
...
# 5g. Insertion
ax_ins = inset_axes(ax, width='30%', height = '25%', loc='lower left', borderpad=3)  # 30% wide and 25% high of the main chart
