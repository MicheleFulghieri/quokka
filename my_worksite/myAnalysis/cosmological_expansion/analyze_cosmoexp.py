...
save_path  = "/data/mfulghieri/quokka/my_worksite/myAnalysis/cosmological_expansion/outputs"
...
for i, plt_path in enumerate(plotfiles):
    ...
    slc.set_zlim(('boxlib', 'gasDensity'), 0.2 * float(mean_rho), 2.2 * float(mean_rho))
...
# For y limits
min_err = np.min(eint_rel_err)
max_err = np.max(eint_rel_err)
max_abs = max(abs(min_err), abs(max_err))  # possibility of negative values
...
ax.yaxis.get_offset_text().set_visible(False)    # hid the exponential y upper left scale
ax.set_xlim(0.8*a_values[0], a_values[-1])
ax.set_ylim(-3 * max_abs, 3 * max_abs)
ax.set_yscale('symlog', linthresh=1e-14)         # symlog to account for positive and negative values,linthresh define the linear region to prevent the log diverge
ax.tick_params(direction='in', which='both')     # ticks point inward
ax.grid(True, which='both', linestyle=':', alpha=0.5)
